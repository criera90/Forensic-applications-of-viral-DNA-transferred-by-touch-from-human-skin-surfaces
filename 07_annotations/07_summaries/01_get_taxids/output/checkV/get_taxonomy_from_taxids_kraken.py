...
def usage():
    usage="""
        Usage:
        ./get_taxonomy_from_taxid_kraken.py -d <file.csv> -t <tax_files_dir> -o <output_dir>
        Options:
...
def get_data(dataset):
    ...
    col_names=["contig", "sciname", "taxid"]
...
for contig in contigs:
    df_temp=df.query('contig == @contig')
    if df_temp.empty == False:
        print(df_temp)
        contigID=df_temp["contig"].iloc[0]
